...
class StreamRouters:
    ...
    async def root_route_handler(self, request):
    ...
    async def old_stream_handler(self, request):
        try:
            message_id = int(request.match_info['message_id'])
            return await self.media_streamer(request, message_id)
        except ValueError as e:
            logging.error(e)
            raise web.HTTPNotFound

    async def media_streamer(self, request, message_id: int):
